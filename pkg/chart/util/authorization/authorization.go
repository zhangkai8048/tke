...
func ConvertTKEAttributesForChart(ctx context.Context, u user.Info, verb string, cg chartv1.ChartGroup, chartName string) (in authorizer.Attributes, err error) {
	attribs := authorizer.AttributesRecord{}
	attribs.User = u

	// If ChartGroup belongs to project, set user extra group
	if cg.Spec.Visibility == chartv1.VisibilityProject && len(cg.Spec.Projects) > 0 {
	...
	attribs.APIGroup = chartv1.GroupName
	attribs.APIVersion = chartv1.Version
...
func ConvertTKEAttributesForChartGroup(ctx context.Context, u user.Info, verb string, cg chartv1.ChartGroup) (in authorizer.Attributes, err error) {
	attribs := authorizer.AttributesRecord{}
	attribs.User = u
...
func AuthorizeForChart(ctx context.Context, u user.Info, authzer authorizer.Authorizer, verb string, cg chartv1.ChartGroup, chartName string) (passed bool, err error) {
...
func AuthorizeForChartGroup(ctx context.Context, u user.Info, authzer authorizer.Authorizer, verb string, cg chartv1.ChartGroup) (passed bool, err error) {
