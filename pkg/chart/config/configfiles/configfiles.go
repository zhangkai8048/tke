...
import (
	...
	"tkestack.io/tke/pkg/chart/config/codec"
	utilfs "tkestack.io/tke/pkg/util/filesystem"
)

// Loader loads configuration from a storage layer
type Loader interface {
	// Load loads and returns the chartconfiguration from the storage layer, or an error if a configuration could not be loaded
	Load() (*chartconfig.chartconfiguration, error)
}
...
type fsLoader struct {
	...
	// registryFile is an absolute path to the file containing a serialized chartconfiguration
	registryFile string
}

// LoadConfigFile returns a chartconfiguration from the `registryFile`
func LoadConfigFile(name string) (*chartconfig.chartconfiguration, error) {
	const errFmt = "failed to load Registry config file %s, error %v"
	// compute absolute path based on current working dir
	chartconfigFile, err := filepath.Abs(name)
	if err != nil {
		return nil, fmt.Errorf(errFmt, name, err)
	}
	loader, err := NewFsLoader(utilfs.DefaultFs{}, chartconfigFile)
	if err != nil {
		return nil, fmt.Errorf(errFmt, name, err)
	}
	kc, err := loader.Load()
...
// NewFsLoader returns a Loader that loads a chartconfiguration from the `registryFile`
...
func (loader *fsLoader) Load() (*chartconfig.chartconfiguration, error) {
	...
	kc, err := codec.Decodechartconfiguration(loader.registryCodecs, data)
	if err != nil {
		return nil, err
	}

	// make all paths absolute
	resolveRelativePaths(chartconfig.chartconfigurationPathRefs(kc), filepath.Dir(loader.registryFile))
	return kc, nil
}
