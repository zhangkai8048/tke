...
import (
	...
	"tkestack.io/tke/pkg/chart/util"
...
func NewHandler(address string, cafile string, loopbackConfig *restclient.Config, chartconfig *chartv1.ChartConfiguration) (http.Handler, error) {
	...
	headers["Authorization"] = "Basic " + base64.StdEncoding.EncodeToString([]byte(
		chartconfig.Security.AdminUsername+":"+chartconfig.Security.AdminPassword),
	)
	cfg := &harbor.Configuration{
		BasePath:      fmt.Sprintf("https://%s/api/v2.0", chartconfig.DomainSuffix),
...
func getChartGroup(tenantID, chartGroupName string) (*registry.ChartGroup, error) {
	...
	// err = chartv1.Convert_registry_ChartGroup_To_v1_ChartGroup(&chartGroup, cg, nil)
	// if err != nil {
