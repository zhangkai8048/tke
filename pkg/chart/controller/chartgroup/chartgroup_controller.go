...
type Controller struct {
	...
	lister         chartv1lister.ChartGroupLister
...
func NewController(businessClient businessversionedclient.BusinessV1Interface,
	client clientset.Interface, chartGroupInformer chartv1informer.ChartGroupInformer,
	resyncPeriod time.Duration, finalizerToken chartv1.FinalizerName, helmClient *helm.APIClient) *Controller {
	...
	chartGroupInformer.Informer().AddEventHandlerWithResyncPeriod(
		cache.ResourceEventHandlerFuncs{
			...
			UpdateFunc: func(oldObj, newObj interface{}) {
				old, ok1 := oldObj.(*chartv1.ChartGroup)
				cur, ok2 := newObj.(*chartv1.ChartGroup)
...
// obj could be an *chartv1.ChartGroup, or a DeletionFinalStateUnknown marker item.
...
func (c *Controller) needsUpdate(old *chartv1.ChartGroup, new *chartv1.ChartGroup) bool {
	if old.UID != new.UID {
		return true
	}
...
func (c *Controller) syncItem(key string) error {
	...
	default:
		...
		if chartGroup.Status.Phase == chartv1.ChartGroupPending ||
			chartGroup.Status.Phase == chartv1.ChartGroupAvailable {
			err = c.processUpdate(context.Background(), key, cachedChartGroup, chartGroup)
		} else if chartGroup.Status.Phase == chartv1.ChartGroupTerminating {
...
func (c *Controller) processUpdate(ctx context.Context, key string, cachedChartGroup *cachedChartGroup, chartGroup *chartv1.ChartGroup) error {
...
func (c *Controller) handlePhase(ctx context.Context, key string, cachedChartGroup *cachedChartGroup, chartGroup *chartv1.ChartGroup) (*chartv1.ChartGroup, error) {
	// newStatus := chartGroup.Status.DeepCopy()
	// switch chartGroup.Status.Phase {
	// case chartv1.ChartGroupPending:
	// 	err := c.createOrDeleteBusinessChartGroup(ctx, cachedChartGroup, chartGroup)
	// 	if err != nil {
	// 		newStatus.Phase = chartv1.ChartGroupFailed
	...
	// 	}
	// 	newStatus.Phase = chartv1.ChartGroupAvailable
	// 	newStatus.Message = ""
	// 	newStatus.Reason = ""
	// 	newStatus.LastTransitionTime = metav1.Now()
	// 	return c.updateStatus(ctx, chartGroup, &chartGroup.Status, newStatus)
	// case chartv1.ChartGroupAvailable:
...
func (c *Controller) createOrDeleteBusinessChartGroup(ctx context.Context, cachedChartGroup *cachedChartGroup, chartGroup *chartv1.ChartGroup) error {
...
func (c *Controller) updateStatus(ctx context.Context, chartGroup *chartv1.ChartGroup, previousStatus, newStatus *chartv1.ChartGroupStatus) (*chartv1.ChartGroup, error) {
	...
	updated, err := c.client.ChartV1().ChartGroups().UpdateStatus(ctx, newObj, metav1.UpdateOptions{})
	if err == nil {
		return updated, nil
	}
	if errors.IsNotFound(err) {
...
func (c *Controller) needCompatibleUpgrade(ctx context.Context, cg *chartv1.ChartGroup) bool {
	// switch cg.Spec.Type {
	// // lower case
	// case chartv1.RepoType(strings.ToLower(string(chartv1.RepoTypePersonal))),
	// 	chartv1.RepoType(strings.ToLower(string(chartv1.RepoTypeProject))),
	// 	chartv1.RepoType(strings.ToLower(string(chartv1.RepoTypeSystem))):
	// 	{
	// 		return true
	// 	}
	// }
...
// If need to upgrade compatibly
func (c *Controller) compatibleUpgrade(ctx context.Context, key string, cachedChartGroup *cachedChartGroup, cg *chartv1.ChartGroup) error {
	// newObj := cg.DeepCopy()
	// switch newObj.Spec.Type {
	// case chartv1.RepoType(strings.ToLower(string(chartv1.RepoTypePersonal))):
	// 	{
	// 		newObj.Spec.Creator = cg.Spec.Name
	// 		newObj.Spec.Type = chartv1.RepoTypeSelfBuilt
	...
	// 		break
	// 	}
	// case chartv1.RepoType(strings.ToLower(string(chartv1.RepoTypeSystem))):
	// 	{
	// 		newObj.Spec.Type = chartv1.RepoTypeSystem
	// 		break
	// 	}
	// default:
	// 	return nil
	...
	// updated, err := c.client.chartv1().ChartGroups().Update(ctx, newObj, metav1.UpdateOptions{})
	// if errors.IsNotFound(err) {
