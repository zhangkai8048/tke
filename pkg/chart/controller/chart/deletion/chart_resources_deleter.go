...
func (d *chartResourcesDeleter) deleteChart(ctx context.Context, chart *chartv1.Chart) error {
...
type updateChartFunc func(ctx context.Context, chart *chartv1.Chart) (*chartv1.Chart, error)

// retryOnConflictError retries the specified fn if there was a conflict error
// it will return an error if the UID for an object changes across retry operations.
// TODO RetryOnConflict should be a generic concept in authClient code
func (d *chartResourcesDeleter) retryOnConflictError(ctx context.Context, chart *chartv1.Chart, fn updateChartFunc) (result *chartv1.Chart, err error) {
	latestChart := chart
	for {
...
func (d *chartResourcesDeleter) updateChartStatusFunc(ctx context.Context, chart *chartv1.Chart) (*chartv1.Chart, error) {
	if chart.DeletionTimestamp.IsZero() || chart.Status.Phase == chartv1.ChartTerminating {
		return chart, nil
	}
	newChart := chartv1.Chart{}
	newChart.ObjectMeta = chart.ObjectMeta
	newChart.Status = chart.Status
	newChart.Status.Phase = chartv1.ChartTerminating
...
func finalized(chart *chartv1.Chart) bool {
	return len(chart.Spec.Finalizers) == 0
}

// finalizeChart removes the specified finalizerToken and finalizes the chart
func (d *chartResourcesDeleter) finalizeChart(ctx context.Context, chart *chartv1.Chart) (*chartv1.Chart, error) {
	chartFinalize := chartv1.Chart{}
	...
	chartFinalize.Spec.Finalizers = make([]chartv1.FinalizerName, 0, len(finalizerSet))
	for _, value := range finalizerSet.List() {
		chartFinalize.Spec.Finalizers = append(chartFinalize.Spec.Finalizers, chartv1.FinalizerName(value))
	}
	chart = &chartv1.Chart{}
	var err error
...
type deleteResourceFunc func(ctx context.Context, deleter *chartResourcesDeleter, chart *chartv1.Chart) error
...
func (d *chartResourcesDeleter) deleteAllContent(ctx context.Context, chart *chartv1.Chart) error {
...
func deleteChart(ctx context.Context, deleter *chartResourcesDeleter, chart *chartv1.Chart) error {
	var errs []error
