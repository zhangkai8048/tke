...
type ChartInfoLister interface {
	...
	List(selector labels.Selector) (ret []*chart.ChartInfo, err error)
...
// List lists all ChartInfos in the indexer.
func (s *chartInfoLister) List(selector labels.Selector) (ret []*chart.ChartInfo, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*chart.ChartInfo))
	})
	return ret, err
}
...
type ChartInfoNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*chart.ChartInfo, error)
...
// List lists all ChartInfos in the indexer for a given namespace.
func (s chartInfoNamespaceLister) List(selector labels.Selector) (ret []*chart.ChartInfo, err error) {
...
// Get retrieves the ChartInfo from the indexer for a given namespace and name.
func (s chartInfoNamespaceLister) Get(name string) (*chart.ChartInfo, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(chart.Resource("chartinfo"), name)
	}
	return obj.(*chart.ChartInfo), nil
}
