...
type ChartGroupLister interface {
	...
	List(selector labels.Selector) (ret []*chart.ChartGroup, err error)
...
// List lists all ChartGroups in the indexer.
func (s *chartGroupLister) List(selector labels.Selector) (ret []*chart.ChartGroup, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*chart.ChartGroup))
	})
	return ret, err
}
...
type ChartGroupNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*chart.ChartGroup, error)
...
// List lists all ChartGroups in the indexer for a given namespace.
func (s chartGroupNamespaceLister) List(selector labels.Selector) (ret []*chart.ChartGroup, err error) {
...
// Get retrieves the ChartGroup from the indexer for a given namespace and name.
func (s chartGroupNamespaceLister) Get(name string) (*chart.ChartGroup, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(chart.Resource("chartgroup"), name)
	}
	return obj.(*chart.ChartGroup), nil
}
